...
<%
Option Explicit
	Dim objReqDet, strUser, retVal, mobj
	Dim rs, deptList, msg, hrRate, dt, introw
	...
	<tr><td>
		<table width="550" border="0" cellspacing="2" cellpadding="5">
			...
			<tr>
				...
				<td> <input name=Developer value="<%=objReqDet.Developer%>" maxlength=100  style="WIDTH: 263px; HEIGHT: 20px" size=37>
			...
			<tr>
				...
				<td><input name="EstManHour" value="<%=objReqDet.EstManHour%>" maxlength=3 style="WIDTH: 116px; HEIGHT: 20px" size=16
					...
					onblur="javascript:GetCostExp('<%=hrRate%>');">
				</td>
			</tr>
			<tr>
  ...
  <table width="550" border="0" cellspacing="2" cellpadding="5">
    ...
    <tr>
      ...
      <td>
		...
		maxlength="4" onKeyPress="if(!((window.event.keyCode >= '48')&&(window.event.keyCode <= '57'))){alert('Please enter NUMERIC values only.');return false;}"
		onblur="javascript:GetCostAct('<%=hrRate%>');">
      </td>
    </tr>
    <tr>
...
<%
	...
	<%Else
		...
		arrcount = Request.Form("delFile").Count
		For i = 2 to arrcount%>

			<INPUT type="hidden" name="delFile" value="<%=Request.Form("delFile")(i)%>">
		<%Next
