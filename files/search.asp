...
<% if Request("order") <> "" then %>document.forms[0].order.value = "<%= Request("order") %>"; <% end if %>
	...
	if (pageAction = "SEARCH") then
		...
		if replace(Request("reqno")," ","")="" then
			intreqno=0
		else
			intreqno=Clng(replace(Request("reqno")," ",""))
		end if
	...
	<table>
		...
		<%
			...
			If cInt(intPage) > 1 Then	%>
		   		<td><a href="search.asp?pageAction=SEARCH&NAV=<%=intPage - 1%>&customer=<%=request("customer")%>&appl=<%=request("appl")%>&status=<%=request("status")%>&appmgr=<%=request("appmgr")%>&teamlead=<%=request("teamlead")%>&developer=<%=request("developer")%>&requestor=<%=request("requestor")%>&fromdate=<%=request("fromdate")%>&todate=<%=request("todate")%>&reqtype=<%=request("reqtype")%>&sortby=<%=request("sortby")%>&order=<%=request("order")%>"><< Prev</a></td>
		   		<td width=10></td>
			<%End IF%>
			<%
			...
			If cInt(intPage) < cInt(intPageCount) Then	%>
		   		<td colspan=2  align="center" ><a href="search.asp?pageAction=SEARCH&NAV=<%=intPage + 1%>&customer=<%=request("customer")%>&appl=<%=request("appl")%>&status=<%=request("status")%>&appmgr=<%=request("appmgr")%>&teamlead=<%=request("teamlead")%>&developer=<%=request("developer")%>&requestor=<%=request("requestor")%>&fromdate=<%=request("fromdate")%>&todate=<%=request("todate")%>&reqtype=<%=request("reqtype")%>&sortby=<%=request("sortby")%>&order=<%=request("order")%>">Next >></a></td>
			<%End If%>
		</tr>
		</table>
		<br>
